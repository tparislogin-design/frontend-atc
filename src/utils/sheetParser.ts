...
export const parseGoogleSheet = async (url: string, startDay: number, endDay: number): Promise<any> => {
  ...
  let csvUrl = url;
  if (url.includes('/edit')) csvUrl = url.replace(/\/edit.*$/, '/export?format=csv');
  else if (!url.includes('export?format=csv')) csvUrl = `${url}/export?format=csv`;
  ...
  try {
    ...
    if (csvText.trim().startsWith('<!DOCTYPE html>')) throw new Error("Le lien n'est pas public.");
      ...
      Papa.parse(csvText, {
        ...
        complete: (results) => {
          ...
          for (let i = 0; i < 20; i++) {
            ...
            if (hasStart && hasNext) { dayRowIndex = i; break; }
          }
          if (dayRowIndex === -1) {
             for (let i = 0; i < 20; i++) {
                if (rows[i].some(cell => cell.trim() === startDay.toString())) { dayRowIndex = i; break; }
             }
          }
          if (dayRowIndex === -1) { reject(`Jour ${startDay} introuvable.`); return; }
          ...
          rows[dayRowIndex].forEach((cell, idx) => {
            ...
            if (!isNaN(val)) {
                ...
                if (isCrossover) { if (val >= startDay || val <= endDay) isIncluded = true; }
                else { if (val >= startDay && val <= endDay) isIncluded = true; }
        ...
        error: (err: any) => { console.error(err); reject("Erreur CSV"); }
      });
    });
